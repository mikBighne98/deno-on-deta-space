    ...
    <div className="w-9/10 max-w-150 mx-auto mt-15">
      ...
      <h1 className="flex items-center justify-between text-5xl font-200">
        <span className="text-white text-xl font-normal">
          A simple Deno app using Aleph.js deployed on Deta
        </span>
        {todos.length > 0 && (
      ...
      <ul className="mt-6">
        {todos.map((todo) => (
          ...
          >
            <div
              className={[
                "flex items-center justify-center w-4.5 h-4.5 border border-gray-100 rounded-full",
                todo.completed && "!border-gray-100/50",
              ]
                .filter(Boolean)
                .join(" ")}
              onClick={() =>
            ...
            >
              {todo.completed && (
                <span className="inline-block w-1.5 h-1.5 bg-gray-200 rounded-full" />
              )}
            </div>
            <label
              className={[
                "flex-1 text-xl text-gray-200 font-300",
            ...
            {todo.id > 0 && (
              ...
              >
                <svg
                  className="w-5 h-5 text-gray-300 hover:text-red-300"
                  viewBox="0 0 32 32"
                  fill="none"
      ...
      >
        <input
          type="text"
          className="block w-full py-2 px-4 text-2xl font-300 placeholder:italic placeholder:text-gray-400 bg-gray-50 rounded-lg outline-none focus:text-neutral-800"
          name="message"
